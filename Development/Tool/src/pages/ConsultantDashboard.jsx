...
export default function ConsultantDashboard() {
  ...
  const fetchTagsMap = async () => {
    const { data } = await supabase.from('tags').select('name, label');
    if (data) {
      const map = {};
  ...
  const fetchCoverageBySector = async () => {
    const { data: sessions } = await supabase
      ...
      .select(`
        id, sector_id,
        sectors!diagnostic_sessions_sector_id_fkey ( name, slug ),
  ...
  const fetchSessions = async () => {
    const { data } = await supabase
      .from('diagnostic_sessions')
      .select(`id, business_name, contact_name, created_at, completed_at, dml_level, total_score, sectors!diagnostic_sessions_sector_id_fkey ( name ), match_results ( result_type )`)
      .eq('status', 'completed')
                ...
                <div className="h-64 mb-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topGrants} layout="vertical" margin={{ left: 50 }}>
                      ...
                      <RechartsTooltip />
                      <Bar dataKey="activation_count" fill="#3B82F6" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="h-64 mb-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topResources} layout="vertical" margin={{ left: 50 }}>
                      ...
                      <RechartsTooltip />
                      <Bar dataKey="match_count" fill="#F97316" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <ul className="space-y-3">
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-surface border-b border-outline-variant text-on-surface-variant">
                    <tr>
                      ...
                      <th className="p-4 font-medium text-center">Grants</th>
                      <th className="p-4 font-medium text-center">Resources</th>
                      <th className="p-4 font-medium text-center">Providers</th>
                  ...
                  <tbody>
                    {recentSessions.map(session => (
                      <tr key={session.id} className="border-b border-outline-variant/50 hover:bg-surface transition-colors">
                        ...
                        <td className="p-4 font-medium">{(session.total_score || 0).toFixed(2)}%</td>
                        <td className="p-4 text-center font-bold text-on-surface">
                          {session.match_results?.filter(r => r.result_type === 'grant').length || 0}
                        </td>
                        <td className="p-4 text-center font-bold text-on-surface">
                          {session.match_results?.filter(r => r.result_type === 'resource').length || 0}
                        </td>
                        <td className="p-4 text-center font-bold text-on-surface">
                          {session.match_results?.filter(r => r.result_type === 'provider').length || 0}
                        </td>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {undercoveredTags.slice(0, 10).map((row, i) => (
                      <tr key={i} className="border-b border-outline-variant/50">
                        <td className="p-3 font-medium text-on-surface">{formatTag(row.tag)}</td>
            ...
            {potentialGrants && (
              <div className="bg-surface-container p-6 rounded-2xl border border-outline-variant">
                ...
                <p className="text-sm text-on-surface-variant mb-4">
                  Based on the top 3 undercovered tags: <span className="font-mono text-xs bg-surface px-2 py-1 rounded">{(potentialGrants.tags_covered || []).map(t => formatTag(t)).join(', ')}</span>
                </p>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={temporalTrends}>
                      ...
                      <YAxis />
                      <RechartsTooltip />
                      <Line type="monotone" dataKey="total_activations" stroke="#3B82F6" strokeWidth={3} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
